...
const DataTbl = (props) => {
  ...
  const rowColor = (idx, lnum) => {
    let rowColor =
      lnum !== "" && lnum !== null && lnum !== undefined
        ? searchParams.get("lotNumber") === lnum
          ? "red"
          : idx % 2 === 0
          ? "#fff"
          : "#ececec"
                ...
                .map((row, RowIndx) => {
                  return (
                    <TableRow
                      hover
                      sx={{
                        backgroundColor: rowColor(RowIndx, row.lotNumber),
                        border: "2px solid #e2e8f0",
                        borderCollapse: "collapse",
                      }}
                      tabIndex={-1}
